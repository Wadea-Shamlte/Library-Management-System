...
namespace LibrarySystem.Book_Copies
{
    public partial class frmAddCopies : Form
    {
        ...
        public frmAddCopies(int BookID)
        {
            InitializeComponent();
        ...
        private void txtNumberOfCopuies_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                e.Cancel = true;
                errorProvider1.SetError(txtBookID, "Number Of Copies cannot be blank");
                return;
            }
            else if (int.Parse(txtNumberOfCopuies.Text) < 1)
            {
                e.Cancel = true;
                errorProvider1.SetError(txtBookID, "Invalid value . Should be addition at least 1 copy.");
                return;
            }
            else
            {
